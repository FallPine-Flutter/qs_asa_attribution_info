...
public class QsAsaAttributionInfoPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(name: "qs_asa_attribution_info", binaryMessenger: registrar.messenger())
    let instance = QsAsaAttributionInfoPlugin()
  ...
  private func attributionTokenResult() -> Any {
    do {
      return try getAttributionToken()
    } catch {
      return FlutterError(
  ...
  private func getAttributionToken() throws -> String {
    guard #available(iOS 14.3, *) else {
      throw QsAsaAttributionInfoError.unsupportedIOSVersion
    }
    return try AAAttribution.attributionToken()
  }

  private func getAttributionInfo(result: @escaping FlutterResult) {
    let token: String
    do {
...
private enum QsAsaAttributionInfoError: LocalizedError {
